...
async def block4(session: Session, player: int):
    player_sum = session.exec(
        select(PlayerSum).where(col(PlayerSum.player) == player)
    ).all()
    tech_top = session.exec(
        select(TechSum)
        .where(col(TechSum.player) == player)
        .order_by(col(TechSum.prozent).desc())
    ).all()
    subtech_top = session.exec(
        select(SubtechSum)
        .where(col(SubtechSum.player) == player)
        .order_by(col(SubtechSum.prozent).desc())
    ).all()
    impact_top = session.exec(
        select(ImpactSum)
        .where(col(ImpactSum.player) == player)
        .order_by(col(ImpactSum.prozent).desc())
    ).all()
    zone_top = session.exec(
        select(ZoneSum)
        .where(col(ZoneSum.player) == player)
        .order_by(col(ZoneSum.prozent).desc())
    ).all()
    return player_sum, tech_top, subtech_top, impact_top, zone_top
